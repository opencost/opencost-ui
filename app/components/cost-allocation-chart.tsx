...
function topNPerDay(
  ...
  const other: AllocationLike[] =
    rest.length > 0 && aggregateAllocs(rest, "other")
      ? [aggregateAllocs(rest, "other")!]
      : [];
...
export default function CostAllocationChart({
  ...
  const aggregateBy =
    aggregateByProp ??
    sharedFilters.drilldownAggregateBy ??
  ...
  const chartData = useMemo(
    () =>
      rawData.length > 0
        ? buildChartData(rawData, topN, includeIdle, accumulate)
        : [],
    [rawData, topN, includeIdle, accumulate],
  );
