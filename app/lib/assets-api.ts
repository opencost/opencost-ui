...
function utilizationFromBreakdown(
  breakdown:
    | {
        idle?: number;
        user?: number;
  ...
  const used =
    (breakdown.user ?? 0) + (breakdown.system ?? 0) + (breakdown.other ?? 0);
  if (idle >= 1 && used === 0) {
    return null;
  }
...
function pickDisplayName(
  ...
  const fromLabels =
    labels?.displayName?.trim() || labels?.hostname?.trim() || "";
...
export function parseAssetsResponse(response: {
  ...
  return Object.entries(response.data)
    .filter(
      ([, asset]) => asset.totalCost !== undefined && asset.type !== undefined,
    )
    .map(([key, asset]) => {
      const labels = asset.labels as Record<string, string> | undefined;
      const propertiesName = String(asset.properties?.name ?? "").trim();
      const name = pickDisplayName(labels, propertiesName) || asset.type || key;

      const internalAddr = labels?.internal_addr?.trim();
      const ipFromName =
        propertiesName && isLikelyIPv4(propertiesName)
          ? propertiesName
          : undefined;
      const ip =
        internalAddr ||
        ipFromName ||
        (typeof asset.ip === "string" ? asset.ip : undefined);
      ...
      return {
        ...
        nodeType:
          typeof asset.nodeType === "string" ? asset.nodeType : undefined,
